...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (cli.getArgList().size() < 1) {
        ...
        if (!Files.exists(inputFile.toPath()) || !Files.isReadable(inputFile.toPath())) {
          System.err.println(
              "The specified input file " + inputFile.getAbsolutePath() + " is not accessable!");
          System.exit(1);
        }
      }
      if (cli.hasOption("o")) {
      ...
      if (cli.hasOption("dot")) {
        String step = cli.getOptionValue("dot", "syntax");
        switch (step) {
          case "syntax":
            dotAfterSyntax = true;
            break;
    ...
    try {
      ...
      compilationUnit = p.parse(); // may thow SyntaxError
    } catch (IOException e) {
      e.printStackTrace();
      System.exit(1);
    }
    ...
    System.out.println("Writing output of syntactical analysis to " + outputFile);
  ...
  private static Options setupCLI() {
    ...
    options.addOption("syn", "syntax-only", false, "(option is ignored)");
    ...
    Option dotOutput =
        Option.builder("dot")
            ...
            .numberOfArgs(1)
            .desc(
                "Output the (D)AST in GraphViz dot-format after the specified compilation step:"
                    + "\n syntax [default]: Output the result of the syntactical analysis as dot-tree")
            .build();
